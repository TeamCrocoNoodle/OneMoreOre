...
const BOSSES: Array[Dictionary] = [
	...
	{"id":"timebomb","title":"황철 시한핵","subtitle":"멈춘 태엽의 금고","goal":Balance.BOSS_ORE_GOALS[3],"reward":Balance.BOSS_REWARDS[3],"radius":3.75,"shape":Vector3.ONE,"layers":[30,14],"health":Balance.BOSS_HEALTH[3],"color":Color("847047"),"accent":Color("ffd57b"),"back":Color("382d19"),"hint":"60초 안에 세 해체선 중 정지선을 찾아 자르세요."},
	{"id":"thorn","title":"가시 흑요석","subtitle":"검은 가시 정원","goal":Balance.BOSS_ORE_GOALS[4],"reward":Balance.BOSS_REWARDS[4],"radius":4.0,"shape":Vector3(1.0,.94,1.0),"layers":[34,16],"health":Balance.BOSS_HEALTH[4],"color":Color("51405d"),"accent":Color("dba1f5"),"back":Color("2b193a"),"hint":"가시가 솟았을 때 직접 공격하면 체력을 잃습니다."},
