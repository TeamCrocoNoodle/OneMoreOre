...
시간 안에 원석을 전부 캐거나 `R`로 교체해도 그 라운드의 남은 시간과 이미 획득한 내역은 유지됩니다. 정산 창은 마우스·터치와 키보드/컨트롤러의 확인 버튼으로 조작할 수 있습니다. 실행 종료 후 저장은 아직 없습니다.
...
채굴 시작 전이나 정산이 끝난 뒤, 왼쪽 아래 `업그레이드` 버튼 또는 `U`로 엽니다. 컨트롤러에서는 `Y / Triangle`을 사용합니다. 현재 HUD와 같은 어두운 바탕·흰색 아이콘·따뜻한 금색 강조를 쓰고, 격자의 대각선과 가로·세로 연결이 고리를 이루는 구조입니다. 무료 시작 노드와 그 이웃 네 개만 처음에 보입니다. 스킬을 구매하면 그 노드의 이웃이 추가로 드러나며, 연결된 이웃 중 하나만 구매해도 접근할 수 있습니다.

노드에는 효과를 나타내는 그림만 표시합니다. 마우스를 올리면 위에는 동전과 가격, 아래에는 이름과 상세 효과가 나옵니다. 노드를 클릭하면 확인 체크 버튼 하나가 나타나고, 이 버튼을 눌러야 Gold를 지불합니다. 다른 곳을 누르면 취소합니다. 터치는 첫 탭으로 상세 내용과 체크 버튼을 표시하며, 키보드·컨트롤러에서도 노드 이동과 확인·취소를 지원합니다. 트리를 보는 동안 채굴 입력과 제한 시간은 시작되지 않습니다.

| 분야 | 스킬 | Gold | 효과 |
| --- | --- | ---: | --- |
| 체력 | 긴 호흡 | 20 | 최대 채굴 시간 30 → 35초 |
| 체력 | 발견의 활력 | 35 | 보석 획득마다 1초 회복, 최대 시간까지 |
| 공격 | 강한 타격 | 25 | 타격 피해 1 → 2 |
| 공격 | 빠른 손놀림 | 40 | 스윙과 연속 공격 속도 +20% |
| 공격 | 넓은 타격 | 55 | 타격점 반경 0.65 안의 보이는 조각 최대 2개 추가 타격 |
| Gold | 보석 감정 | 30 | 모든 보석의 정산 단가 +20%, 돌 조각 단가는 유지 |
| 원석 | 풍부한 광맥 | 40 | 원석마다 일반 보석 3 → 4개, 특별 보석 1개 유지 |
| 원석 | 무른 돌 | 50 | 일반 돌 조각의 체력 -1, 최소 1; 보석을 품은 조각은 16 유지 |

각 스킬은 한 번 구매할 수 있습니다. 정산 후 구매한 효과는 다음 채굴부터 적용합니다. 아직 캐지 않은 시작 원석에서 원석 관련 스킬을 구매하면 현재 원석도 새 구성으로 바뀝니다. 넓은 타격은 파괴 전에 보이던 조각만 대상으로 삼으므로, 같은 타격에서 새로 드러난 안쪽 조각을 연속으로 관통하지 않습니다.
...
도구 탭의 진열대는 레퍼런스처럼 굵은 중앙 기둥으로 나뉜 좌우 두 진열칸과 3단 선반을 가진 실제 3D 모델입니다. 재질은 짙은 철제로 바꾸고, 깊은 측판·철판 뒤벽·접힌 선반 테두리·육각 볼트·금속 가격 명판을 만들었습니다. 정면에서 조금 내려다보는 원근 카메라를 사용해 바깥 기둥이 아래로 갈수록 안쪽으로 모이고, 아래쪽 선반의 윗면이 더 넓게 보이도록 맞췄습니다. 기존 곡괭이 메시의 강철·구리·은빛·코발트·흑철·황금 변형 여섯 개가 선반에 놓이며, 도구와 철제 진열대는 같은 조명과 그림자를 받습니다. 명판 글자와 클릭 영역도 실제 모델의 투영 위치를 따라갑니다. 좁은 화면에서도 좌우 칸을 유지하고, 높이가 부족하면 휠·터치패드·터치 드래그로 스크롤할 수 있습니다.

현재 도구와 가격은 진열 화면을 위한 임시 구성입니다. 기존 곡괭이는 `사용 중`, 나머지는 가격 옆에 `준비 중`으로 표시합니다. 도구 선택은 진열 강조만 바꾸며, 구매·장착·Gold 차감이나 채굴 능력 변경은 하지 않습니다. 진열대 전체를 하나의 미리보기로 렌더링하며, 첫 진입이나 화면 배치가 달라질 때만 갱신합니다. 스크롤·호버에는 기존 렌더 결과를 재사용하고, 탭을 닫으면 진열대 입력 처리를 중지합니다.

Gold 표시는 두께·테두리·양각 G 문양을 가진 3D 동전을 사용합니다. 지갑, 스킬 비용, 도구 가격표, 정산 금액에 같은 모델을 연결했습니다. 보석함·정산·스킬 안의 보석 그림도 채굴에서 사용하는 실제 보석 메시로 통일했습니다. 동전 하나와 여섯 등급 보석은 한 번만 렌더링한 결과를 UI 전체에서 공유하므로, 표시 개수가 늘어도 별도 3D 렌더링을 반복하지 않습니다.
...
처음 등장하는 기본 원석에는 **일반 보석 3개와 특별 보석 1개**가 들어갑니다. 풍부한 광맥을 구매하면 일반 보석이 1개 늘어납니다. 보석의 희귀도와 최종 균열 빛 색은 다음과 같습니다.
...
돌·곡괭이·보석·파편은 실행 시 생성하는 원본 3D 메시입니다. 참고 이미지 파일이나 외부 유료 에셋은 포함하지 않습니다. 일반 돌의 채굴·파괴음과 보석 돌의 타격·발견음은 제공된 `Mining_sound_Ref.wav`와 `Ore Sound Ref_.wav`의 타격감·금속성 울림을 참고해 [새로 합성한 WAV](assets/audio/README.md) 8개입니다. 원본 녹음의 구간이나 음악·잡음은 사용하지 않습니다. 48 kHz 스테레오 PCM 16-bit로 미리 제작해 불러오며, 보석 타격은 짧고 밝은 울림, 발견은 연속 금속 타격과 마지막 울림으로 구분합니다. 곡괭이 휘두르기·돌 재생성 소리와 보석함 도착·카운트다운·정산 효과음은 시작할 때 합성해 재사용합니다. 보상 효과음은 배경 음악 없이 짧은 종소리와 코인 소리로 구성한 24 kHz 모노 PCM 16-bit이며, 타격 중에는 오디오를 합성하거나 새 재생 노드를 만들지 않습니다.
...
스킬 검증은 인접 노드 공개, 잔액·중복 구매 차단, 실제 확인 버튼·취소·컨트롤러 입력과 여덟 효과의 채굴·정산 반영을 확인합니다. 넓은 타격으로 앞 조각을 부숴도 같은 타격에서 뒤 조각이 피해를 받지 않는지, 창을 닫은 뒤 입력과 정산 버튼 포커스가 올바르게 돌아오는지도 검사합니다. `capture_skills.gd`는 실제 UI 구매 과정을 `artifacts/skills_*.png`에 저장합니다. 이 화면 검증에서만 지갑에 160G를 넣어 구매 흐름을 확인하며, 실제 게임의 시작 지갑은 0G입니다.
