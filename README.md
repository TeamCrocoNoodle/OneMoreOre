...
도구 탭의 나무 진열대는 두꺼운 선반·모서리를 다듬은 기둥·뒤판·쇠못으로 만든 실제 3D 모델입니다. 기존 곡괭이의 메시를 재사용해 강철·구리·은빛·코발트·흑철·황금 색상으로 여섯 개를 진열하며, 도구와 선반은 같은 공간에서 조명과 그림자를 받습니다. 가격표는 각 선반의 실제 위치에 맞춰 끈으로 매달았습니다. 가로 화면은 3열, 좁은 화면은 2열이고, 높이가 부족하면 휠·터치패드·터치 드래그로 선반을 스크롤할 수 있습니다.
