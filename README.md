...
빛줄기는 모든 실제 균열 선분에서 넓은 사다리꼴 면으로 펼쳐집니다. 밑변 양 끝은 금의 양 끝에 붙으며, 금의 옆 방향으로 빛이 뻗어 선 모양의 틈에서 새어 나오는 모습입니다. 각 광선의 투명도를 높이고 균열 수에 따라 밝기를 조절해, 빛이 겹쳐도 돌 표면이 비쳐 보입니다. 곧게 뻗은 양쪽 경계와 거리에 따라 옅어지는 색, 균열 주변의 빛 번짐과 작은 마름모 반짝임이 타격의 빛을 받쳐 줍니다.
...
돌·곡괭이·보석·파편은 실행 시 생성하는 원본 3D 메시입니다. 참고 이미지 파일이나 외부 유료 에셋은 포함하지 않습니다. 채굴·파괴음은 제공된 `Mining_sound_Ref.wav`에서 추출한 [WAV 샘플](assets/audio/README.md)을 미리 불러와 재생합니다. 곡괭이 휘두르기·보석 공명·획득·돌 재생성 소리는 시작할 때 합성합니다. UI, 판매, 업그레이드, 저장 기능은 이 프로토타입 범위에 포함하지 않습니다.
